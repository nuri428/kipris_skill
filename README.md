...
| 호출 횟수 초기화 | 매월 1일 |
| 미신청 API 호출 | 응답 `resultCode: 101` / `AccessKey&ServiceID Is Not Registerd Error` |

> **Note**: 무료 가입자는 월 1,000회까지 API 호출이 가능합니다. 초과 사용이 필요한 경우 [KIPRIS Plus](https://plus.kipris.or.kr)에서 유료 플랜을 확인하세요.
>
> **주의**: 유료 플랜 사용 시에도 별도로 신청한 API만 호출할 수 있습니다. 신청하지 않은 API를 호출하면 `resultCode: 101` (`AccessKey&ServiceID Is Not Registerd Error`)과 같은 에러가 반환될 수 있으므로, 사용할 서비스를 KIPRIS Plus 포털에서 미리 신청해야 합니다.
...
사용자 요청 → Claude Code → SKILL.md (오퍼레이션 매핑)
                                ↓
                        web_fetch (KIPRIS API 호출)
                                ↓
                        XML 응답 수신
                                ↓
                        Claude가 직접 파싱 및 정리 → 사용자에게 테이블로 제공
```
